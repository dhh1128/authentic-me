...
I use the label `daniel-as-public-person` to refer to it. If you run into that label and my identity is the context, this is the identifier I'm talking about. However, this label is just a convenience that's not globally unique, and in your local world, you can label it however you like. It is the identifier, not its label, that matters when analyzing security.

## Inception
The value of this identifier is generated by the [bytewise and externalized saidify algorithms](https://dhh1128.github.io/papers/bes.pdf), from a CESR-encoded Blake3-256 hash of the file content at its **inception state**. The inception state is not necessarily the first commit of the file (since I'm writing this by hand, in stages). Rather, it is the commit in [this file's commit history](https://github.com/dhh1128/authentic-me/commits/main/identifiers/EGZ_DdmzryjQOtOdQauTm_YxggbVM7EWelk8IBxsnC-d.md) when event 0 is first defined in the [Key Event Log](#key-event-log) section, and when the identifier value is copied into the filename/URL and file content. To prevent me or anyone else from forking this file just before a given commit and evolving a new history, I am using a [strategy for witnesses and watchers](../witnesses-and-watchers.md).
